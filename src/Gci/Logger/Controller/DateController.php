...
class DateController {
    ...
    public function indexActionGet($request, $response, array $args) {
        ...
        $getTime = $request -> getParam('getTime');
        ...
        $statement = null;
        $sql = ' SELECT * FROM `request` WHERE `createdAt` <="'.$date.' '.$getTime.'" AND `status`'.$statusValue.' ORDER BY id DESC'; 
        ...
        $statement -> execute();
        $dateResults = $statement -> fetchAll();
        $statement = null;
     
       // total
        $statement = null;
        $statement = $pdo -> prepare('SELECT COUNT(*) AS total FROM `request` WHERE `createdAt` <="'.$date.' '.$getTime.'" AND `status`'.$statusValue.' ORDER BY id DESC'); 
        $statement -> bindValue('date', "%".$date."%"); 
        $statement -> execute();
        ...
        // Render index view
        return $this->container->get('renderer')->render($response, 'date/index.phtml', array(
